"""HQMQ + naive baselines + outlier-extraction sweep on Qwen3-8B.

Qwen3-8B: 36 layers, 8 KV heads, d_head=128 (GQA, HQMQ-compatible).
Tests whether the outlier-extraction story persists into the next Qwen generation.
"""
...
def main():
    ...
    hqmq_configs = [
        ...
        ("s48",  48,  4, False),  # 3.54
        ("s96",  96,  4, False),  # 3.79
        ("s192", 192, 4, False),  # 4.04
        ("s192", 192, 6, False),  # 4.54
    ]
    for tag, s, rb, _ in hqmq_configs:
        name = f"hqmq_{tag}_r{rb}"
        print(f"\n=== {name} ===", flush=True)
        q = HQMQQuantizer(n_layers=n_layers, n_heads=n_heads,
                          secondary_size=s, radius_bits=rb, device=device, init="random")
        run(name, q)

    # Med3× variants (in case Qwen3 also has outliers)
    for tag, s, rb in [("s24", 24, 6), ("s96", 96, 6), ("s192", 192, 6)]:
        name = f"hqmq_{tag}_r{rb}_Med3"
        print(f"\n=== {name} ===", flush=True)
    ...
    out_path = out_dir / f"35_qwen3_sweep_{int(time.time())}.json"
