"""HQMQ + Med3x sweep on Microsoft Phi-3.5-mini (and optionally Phi-3-medium).

Phi-3.5-mini-instruct: 32 layers, 32 KV heads (full MHA, no GQA), d_head=96.
d_head=96 IS divisible by 4 → standard HQMQ pipeline applies, no padding needed.

Tests cross-vendor generalization: another tech-major architecture, MHA not GQA,
slightly different d_head than Mistral/Llama/Qwen.
"""
...
def main():
    ...
    p.add_argument("--model", default="microsoft/Phi-3.5-mini-instruct")
    ...
    # HQMQ Pareto sweep
    for s, rb in [(24, 3), (48, 4), (96, 4), (192, 4), (192, 6)]:
    ...
    out_path = out_dir / f"39_phi_sweep_{int(time.time())}.json"
